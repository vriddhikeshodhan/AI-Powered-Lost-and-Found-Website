...
async def run_rematch_job():
    ...
    try:
        ...
        # ── Step 1: Fetch all active lost items (WITH latitude and longitude) ─────────
        cur.execute("""
            SELECT
                i.item_id,
                i.user_id,
        ...
        for lost in lost_items:
            ...
            # ── Step 2: Fetch candidate found items with 1KM HAVERSINE FILTER ─────
            ...
            if has_location:
                query = """
                    ...
                    WHERE i.type        = 'Found'
                      ...
                      AND (
                          ...
                          OR (
                              6371000 * 2 * ASIN(SQRT(
                                  POWER(SIN(RADIANS(i.latitude  - %s) / 2), 2) +
                                  COS(RADIANS(%s)) * COS(RADIANS(i.latitude)) *
                                  POWER(SIN(RADIANS(i.longitude - %s) / 2), 2)
                              ))
                          ) <= 1000
                      )
                      AND NOT EXISTS (
                ...
                """
                params = (category_id, lost_user_id, lost_lat, lost_lat, lost_lng, lost_id)
            else:
                query = """
                    SELECT
                        i.item_id,
                ...
                """
                params = (category_id, lost_user_id, lost_id)
            ...
            for found_item in candidates:
                ...
                # ── Step 3: Compute scores (with NumPy safe-checks) ───────────
                ...
                # FIXED: Access dictionary using string literal 'match_id'
                ...
                """, (
                    ...
                    lost_id,
                    match_id,
                    f'A possible match has been found for your lost item: "{lost_title}". Confidence: {confidence}%'
                ))
...
def _cosine_similarity(vec_a, vec_b) -> float:
    ...
    # FIXED: Explicitly check for None to prevent NumPy ambiguity errors
